...
def get_model_complexity_info(model, npoints,
    ...
    pc = torch.randn((1, npoints, 3), 
        dtype=next(flops_model.parameters()).dtype,
        device=next(flops_model.parameters()).device)
    mask = torch.ones((1, npoints),
        dtype=next(flops_model.parameters()).dtype,
        device=next(flops_model.parameters()).device).type(torch.int32)
    features = torch.randn((1, 3, npoints), 
        dtype=next(flops_model.parameters()).dtype,
        device=next(flops_model.parameters()).device)
