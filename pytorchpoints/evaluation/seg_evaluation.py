...
class SegEvaluator(DatasetEvaluator):
    ...
    def transformer(self, cfg, data, TS, vote_idx):
        if vote_idx > 0:
            for d in data:
                points = d[kfg.POINTS]
                if vote_idx > 0:
    ...
    def one_vote(self, cfg, model, test_data_loader, TS, vote_idx):
        all_logits = []
        all_points_labels = []
        all_shape_labels = []
        
        for idx, data in enumerate(test_data_loader):
            data = self.transformer(cfg, data, TS, vote_idx)
    ...
    def evaluate(self, cfg, model, test_data_loader, epoch):
        ...
        for v in range(num_votes):
            all_logits, all_points_labels, all_shape_labels = self.one_vote(cfg, model, test_data_loader, TS, v)
