...
class RotInvRRISep(nn.Module):
    @configurable
    def __init__(self, nsample, radius, dims, norm_last, act_last, reduction, init_gamma1, init_gamma2):
        super(RotInvRRISep, self).__init__()
        self.grouper = MaskedQueryAndGroup(radius, nsample, use_xyz=False, ret_grouped_xyz=False, normalize_xyz=False)
        self.mask_val = 10000
        self.rri_init_dim = 19
        self.radius = radius
    ...
    def xyz_to_rri_v1(self, xyz, knn_mask, radius=0.05, eps=1e-8, point_start = 1): # B, N, K, 3
        ...
        neighbor_point_xyz = torch.cat([
            neighbor_reference_vector / radius
        ], dim=-1)
        rel_pos = neighbor_point_xyz
        ...
        rri = torch.cat([neighbor_point_xyz, neighbor_point_rri_z, center_point_features, neighbor_point_features], dim=-1)
    ...
    def forward(self, xyz, mask):
        ...
        with torch.no_grad():
            feats = self.xyz_to_rri_v1(knn_xyz, knn_mask, self.radius)
        feats = self.feats_bn(feats)

        feats1 = feats[:, 0:3] * self._gamma1
        feats2 = feats[:, 3:7] * self._gamma2
        feats3 = feats[:, 7:]
        rri = torch.cat([feats1, feats2, feats3], dim=1)
